## TO DO - clean up formatting on mleg saved files
        ...
        if len(opt_parents) > 0:
            ...
            for opt in opt_parents:
                ...
                if fills.loc[fills.index[0], 'execShape'] == 'Single':
                    ...
                    result_dict = {leg: {'side': '', 'qty': 0, 'results': None} for leg in fills['optName'].unique()}
                    for i, leg in enumerate(result_dict.keys()):
                        legFills = fills[fills['optName'] == leg]
                        results = calc_TCA_metrics(legFills, None, None, arrActSlipPct, False)
                        fName = f'{dt:%Y%m%d} {opt}-{i+1}.csv'
                        results.to_csv(os.path.join(os.getcwd(), 'TCA', fName))
                        wins += 1
                        result_dict[leg] = {'side': legFills.loc[legFills.index[0], 'orderSide'],
                                            'qty': legFills['fillQuantity'].sum(),
                                            'results': results}
                    opt_str = ''
                    sum_results = None
                    no_sum = ['Order',
                    ...
                    for k in result_dict.keys():
                        qty = result_dict[k]['qty']
                        if 0 < qty < min_qty:
                            min_qty = qty
                        if result_dict[k]['side'] == 'Buy':
                            mult = qty
                        else:
                            mult = -qty
                        opt_str += k + ' '
                        df = result_dict[k]['results']
                        if sum_results is None:
                            sum_results = df.copy()
                            sum_results.loc[~df.index.isin(no_sum), ['Maker', 'Taker', 'Total']] = 0

                        sum_results.loc[~df.index.isin(no_sum), ['Maker', 'Taker', 'Total']] += \
                            df.loc[~df.index.isin(no_sum), ['Maker', 'Taker', 'Total']] * mult

                    sum_results.loc[~df.index.isin(no_sum), ['Maker', 'Taker', 'Total']] /= min_qty
                    sum_results.iloc[0, 3] = opt_str
                    fName = f'{dt:%Y%m%d} {opt}-Cons.csv'
